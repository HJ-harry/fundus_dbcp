...
from dcp.boxfilter import boxfilter2d
...
def myGuidedFilter(img, p, r=60, eps=1e-4):
    mean_I = box_blur(img, (r, r))
    mean_p = box_blur(p, (r, r))
    mean_Ip = box_blur(img*p, (r, r))
    cov_Ip = mean_Ip - mean_I * mean_p

    mean_II = box_blur(img*img, (r, r))
    var_I = mean_II - mean_I * mean_I

    a = cov_Ip/(var_I + eps)
    b = mean_p - a*mean_I

    mean_a = box_blur(a, (r, r))
    mean_b = box_blur(b, (r, r))
